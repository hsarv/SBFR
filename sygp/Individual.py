...
class Individual:
	...
	def createTerminals(self, i=0):
		self.dimensions = []
		n = Node()
		n.createTerminals(self.operators, self.terminals, self.max_depth, full=True, i=i)
		self.dimensions.append(n)
	...
	def setm_iy(self, m_y):
		self.mi_y = m_y

	def setm_info(self, m_info):
		self.m_info = m_info
	...
	def getAge(self):
		'''
		Returns the age of an individual.
		'''
		if not self.Age:
			self.size = 0
		return self.Age
	...
	def set_semantic(self,X):
		self.semantic = self.convert(X)
